...
for filter_index in range(0, 32):
    # we only scan through the first 4 filters,
...
# we will stitch the best 16 filters on a 2 x 2 grid.
n = 4

# the filters that have the highest loss are assumed to be better-looking.
# we will only keep the top 4 filters.
kept_filters.sort(key=lambda x: x[1], reverse=True)
kept_filters = kept_filters[:n * n]

# build a black picture with enough space for
# our 2 x 2 filters of size 128 x 128, with a 5px margin in between
margin = 5
